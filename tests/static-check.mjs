...
const required = [
  ...
  "reset.html",
  "BUILD_ID.txt",
  "docs/SOURCE_OF_TRUTH_CM8.md",
  "docs/IMPLEMENTATION_CM8P.md"
];
...
for (const token of [
  "CM8P",
  "deriveMessageKeyFromPattern",
  "encryptWithPatternKey",
  "pattern-bound-capsule"
]) {
  if (!app.includes(token)) fail("Missing CM8P implementation token: " + token);
}
...
for (const token of ["כתוב", "סימן", "הודעה", "פתח", "קוד", "השב"]) {
  if (!(app + html + config).includes(token)) fail("Missing expected Hebrew UI token: " + token);
}
...
for (const token of [
  ...
  if (!app.includes(token)) fail("Missing Magic state-machine contract token: " + token);
}
...
for (const forbidden of [
  ...
  if ((app + html).includes(forbidden)) fail("Forbidden Magic UX regression token found: " + forbidden);
}

for (const bad of ["SM5:eyJ", "lemon-fish-lock", "4/240"]) {
  if ((html + app + config).includes(bad)) fail("Forbidden legacy token found: " + bad);
}
for (const bad of ["SecretMoji"]) {
  if ((html + manifest).includes(bad)) fail("Forbidden visible legacy brand found: " + bad);
}
...
for (const [file, content] of scanned) {
  for (const marker of mojibakeMarkers) {
    if (marker.test(content)) fail("Mojibake marker found in " + file + ": " + marker);
  }
}

console.log("static-check: PASS CM8P Magic State Machine");
